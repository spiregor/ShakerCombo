...
function Shaker.ITABcombo()
	...
	local mousePos = Input.GetWorldCursorPos()
	 ...
	 if Ability.IsCastable(Slam, myMana) and NPC.IsPositionInRange(myHero, NPC.GetAbsOrigin(hero),1200,0) and blink and Ability.IsCastable(blink, myMana) then
	...
	if refresher and Menu.IsEnabled(Shaker.refreshEnable) and Ability.IsCastable(refresher,myMana) then Ability.CastNoTarget(refresher) return end
